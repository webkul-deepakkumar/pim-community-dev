...
SQL;
     ...
     * @return JobExecutionMessage|null
     */
    public function getFilteredAvailableJobExecutionMessage(array $jobInstanceCodes): ?JobExecutionMessage
    {
        $sql = <<<SQL
SELECT 
    q.id, q.job_execution_id, q.create_time, q.updated_time, q.options, q.consumer
...
//        $stmt = $this->entityManager->getConnection()->prepare($sql);
//        $stmt->bindValue('job_instance_codes', $jobInstanceCodes, Connection::PARAM_STR_ARRAY);
//        $stmt->execute();
        $row = $stmt->fetch();
